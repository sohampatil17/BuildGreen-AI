...
class EnergyStarComplianceChecker:
    def __init__(self):
    ...
    def upload_pdf(self):
        uploaded_file = st.file_uploader("Upload your Energy Star report PDF", type=["pdf"])
        if uploaded_file is not None:
            # Display the PDF
            st.success('PDF uploaded successfully!')
            st.write('Your Energy Star report PDF:')
    ...
    def analyze_compliance(self, text):
        try:
            response = openai.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an AI trained to help with Energy Star compliance."},
    ...
    def run(self):
        st.title("Energy Star Compliance Checker")
        ...
        with col2:  # Content for the second column (Checklist and tracker)
            st.subheader("Energy Star Compliance Checklist")
            ...
            "Employee or occupant engagement in energy conservation",
            "Regular energy audits and assessments",
            "Maintenance of energy systems for optimal performance",
            "Investment in energy-saving technologies",
            "Continuous improvement of energy performance",
            "Compliance with local and national energy efficiency standards",
            "Energy-efficient landscaping and exterior lighting",
...
if __name__ == "__main__":
    checker = EnergyStarComplianceChecker()
    checker.run()
